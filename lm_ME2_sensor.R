...
outliers = alldata %>%
  ...
  identify_outliers(log_X1.11Hz) # change to X1.11Hz, X1.67Hz and X3.3Hz
...
summary(lmall) 
lmall_noAge = lmer(X1.67Hz ~ age*condition-age:condition + (1|sub_id),data= alldata,verbose=2,subset = condition != "_04")  
anova(lmall,lmall_noAge)

# triple lmer
lmall = lmer(X1.11Hz ~ age*condition  + (1|sub_id),data= alldata,verbose=2,subset = condition != "_03")  
summary(lmall) 
lmall_noAge = lmer(X1.11Hz ~ age*condition-age:condition + (1|sub_id),data= alldata,verbose=2,subset = condition != "_03")  
anova(lmall,lmall_noAge)
